...
public class AlgoHAEIMiner {
        ...
        try {
            ...
            while ((thisLine = myInput.readLine()) != null) {
                ...
                for(int i = 0;i < revisedTransaction.size(); i++){
                    ...
                    //将Element值存入对应的列表中
                    Element element = new Element(tid, pair.utility, rmu);
    ...
    private void HAEIMine(int[] prefix,UtilityList pUL,List<UtilityList> ULs,double min_AE)
        ...
        for (int i = 0; i < ULs.size(); i++){
            ...
            double Mae =0 ;
            if(i+1< ULs.size()) {
                long invnext = mapItemToINV.get(ULs.get(i+1).item);
                if(X.mn == 0) Mae = 0;
                else{
                    Mae = calMae(X, length, invnext);
                }
            }
    ...
    private double calMae(UtilityList ul, int length,long invnext){
        double sumMau = 0;
        ...
        for(int i = 0;i<ul.elements.size();i++){
            ...
            double tmae;
            double invNew = (double)(ul.inv + invnext);
            if (element.rmu != 0) {
                if(element.rmu > au) {
                    tmae = (double) (element.iutils + element.rmu * ul.mn) / (invNew * (length + ul.mn));
                }else {
    ...
    private UtilityList construct(UtilityList P,UtilityList px,UtilityList py){
        ...
        for(Element ex : px.elements){
            ...
            if (P == null){
                //填入新的效用列表中
                Element eXY = new Element(ex.tid, ex.iutils + ey.iutils, ey.rmu);
                ...
                if (e != null) {
                    //填入新的效用列表中
                    Element eXY = new Element(ex.tid, ex.iutils + ey.iutils - e.iutils, ey.rmu);
                    pxyUL.addElement(eXY, py.getMn());
                }
            }
        }
