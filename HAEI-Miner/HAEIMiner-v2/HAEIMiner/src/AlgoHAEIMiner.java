...
public class AlgoHAEIMiner {
       ...
       //用于获得当前系统时间，记为算法开始时间
        ...
        try {
            ...
            while ((thisLine = myInput.readLine()) != null) {
                ...
                //将revisedTransaction按照AEUB值进行升序排序
                ...
                for(int i = 0;i < revisedTransaction.size(); i++){
                    ...
                    //将Element值存入对应的列表中
                    Element element = new Element(tid, pair.utility, rmu, rmi);
    ...
    private UtilityList construct(UtilityList P,UtilityList px,UtilityList py){
        ...
        for(Element ex : px.elements){
            ...
            if (P == null){
                //填入新的效用列表中
                Element eXY = new Element(ex.tid, ex.iutils + ey.iutils, ey.rmu, ey.rMinInv);
                ...
                if (e != null) {
                    //填入新的效用列表中
                    Element eXY = new Element(ex.tid, ex.iutils + ey.iutils - e.iutils, ey.rmu, ey.rMinInv);
                    pxyUL.addElement(eXY, py.getMn());
                }
            }
        }
